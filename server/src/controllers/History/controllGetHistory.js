...
const controllGetHistory = async (userId) => {
    ...
    const responsePromises = userHistory.map(async (user) => {
        ...
        const color = await Colors.findByPk(stock.dataValues.ColorId, {attributes: ["codHex", "name"]});

        const size = await Sizes.findByPk(stock.dataValues.SizeId, {attributes: ["name"]});

        const product = await Products.findByPk(stock.dataValues.ProductId, {attributes: ["mainImage", "name"]});
